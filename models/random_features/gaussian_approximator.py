...
from rf_util.measures import Fixed_Measure, Exponential_Measure, P_Measure
from rf_util.kernels import gaussian_kernel
import rf_util.data
...
if __name__ == "__main__":
    ...
    data = rf_util.data.load_dataset('config/datasets/mnist.json', standardize=True, normalize=False)
    data_name, train_data, test_data, train_labels, test_labels = data

    data = train_data[torch.randperm(len(train_data))][:1000]
    data = rf_util.data.pad_data_pow_2(data)[:, :d]
