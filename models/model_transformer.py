...
from models.attention import Attention, MaclaurinAttention
...
class MaclauformerLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.mha = MaclaurinAttention(config)
        self.dropout1 = torch.nn.Dropout(p = config["dropout_prob"])
        self.norm2 = nn.LayerNorm(config["transformer_dim"])
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.former = {
            ...
            'maclauformer':MaclauformerLayer
        }[config["former"]]
        # print('former',self.former)
        self.embeddings = Embeddings(config)

        if self.tied_weights:
            self.transformer = self.former(config)
        else:
            for idx in range(self.num_layers):
                setattr(self, f"transformer_{idx}", self.former(config))
